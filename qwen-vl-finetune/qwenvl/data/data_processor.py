...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):
        ...
        for data in dataset_list:
            ...
            if data_args.biased_sampling:
                nested_list_data_dict.append(annotations)

        rank0_print(f"Total training samples: {len(list_data_dict)}")

        random.shuffle(list_data_dict)  # Randomly shuffle the data for training
        if data_args.biased_sampling:
            [random.shuffle(sublist) for sublist in nested_list_data_dict]
        ...
        self.nested_list_data_dict = nested_list_data_dict
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                if self.data_args.biased_sampling:
                    num_tasks = len(self.nested_list_data_dict)
                    current_task_list = self.nested_list_data_dict[i % num_tasks]
                    sample_idx = (i // num_tasks) % len(current_task_list)
                    sources = current_task_list[sample_idx]
                else:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                if self.data_args.biased_sampling:
                    num_tasks = len(self.nested_list_data_dict)
                    current_task_list = self.nested_list_data_dict[
                        next_index % num_tasks
                    ]
                    sample_idx = (next_index // num_tasks) % len(current_task_list)
                    sources = current_task_list[sample_idx]
                else:
